...
import { captureIdFromRouteQuery, clearStarCaptureRouteQuery, isCurrentStarCapture, isMainOnlyStarCaptureEvent, isStarCaptureReadyEvent, loadAndImportStarCapture } from "./captureTransport.js";
...
function mainOnlyTransportSmokeEnabled() {
  return import.meta.env.DEV && route.query.transport_smoke === "1";
}
...
function queueCapture(captureId, allowMainOnlyTransportSmoke) {
  ...
  if (!pendingCapture || pendingCapture.captureId !== normalizedCaptureId || pendingCapture.accountId !== currentAccountId) {
    pendingCapture = { accountId: currentAccountId, captureId: normalizedCaptureId, batch: null, allowMainOnlyTransportSmoke: allowMainOnlyTransportSmoke === true };
...
async function recoverPendingCapture() {
  ...
  try {
    ...
    queueCapture(pending.capture_id || pending.captureId, import.meta.env.DEV && String(pending.section || "") === "main");
  } catch (error) {
...
function onStarCaptureEvent(event) {
  ...
  queueCapture(event.data.capture_id || event.data.captureId, import.meta.env.DEV && isMainOnlyStarCaptureEvent(event));
}
...
watch(function () { return [route.query.capture_id, route.query.account_id, route.query.transport_smoke, productReady.value, accountId.value]; }, queueRouteCapture);
